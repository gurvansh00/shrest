...
def calculate_angle3(e,f,h):
    np.array(e)
    np.array(f)
    np.array(h)
    radians = np.arctan2(h[1]-f[1], h[0]-f[0]) - np.arctan2(e[1]-f[1], e[0]-f[0])
...
def calculate_angle4(b,c,i):
    np.array(b)
    np.array(c)
    np.array(i)
    radians = np.arctan2(i[1]-c[1], i[0]-c[0]) - np.arctan2(b[1]-c[1], b[0]-c[0])
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # Calculate angle
            angle1 = calculate_angle1(wristL, elbowL, shoulderL)
            angle2 = calculate_angle2(wristR, elbowR, shoulderR)
            angle3 = calculate_angle3(elbowR, shoulderR, hipR)
            angle4 = calculate_angle4(elbowL, shoulderL, hipL)
            print('left angle elbow',angle1)
            print('right angle elbow',angle2)
            print('right angle shoulder',angle3)
            print('left angle shoulder',angle4)
            ...
            cv2.putText(image, str(angle4), 
             ...
             # Curl counter logic
            if angle1+angle4 >160  :
                stage = "down"
            if angle1+angle4 <75  and stage =='down':
                stage="up"
                counter +=1
                print(counter)
